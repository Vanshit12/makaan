...
const Files = (props) => {
    ...
    const handlePageClick = (event) => {
        Makaan.get(`properties?page=${event.selected+1}&type=File`)
			  .then(res => {
    ...
    useEffect(() => {
        ...
        return (
            <MultipleProperties unit={true} handlePageClick={handlePageClick} total={total} list={state.houses_list} heading={t('TOP DEALS ON FILES')}/>
        );
}
