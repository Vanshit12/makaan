...
const Home = () => {
	const navigate = useNavigate();
		...
		// 	Makaan.get(`property-type`).then(res => {
		// 		setState({...state, property_types:res.data})
    	...
    	useEffect(() => {
			...
			axios.all(endpoints.map((endpoint,i) => {
				...
				return Makaan.get(endpoint);
					...
					setState({...state, property_types: res[0].data, cities:res[1].data })
					console.log(res[0].data,'uhihiu')
				}
			);
		},[]);
