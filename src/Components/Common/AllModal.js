...
const AllModal = ({show, handleClose, showWhatsapp, handleCloseWhatsapp, showRequest, handleCloseRequest}) => {
    ...
    const phninref = useRef(null);
    const phnuaeref = useRef(null);
    const whatsappref = useRef(null);
    const copyToClipboard = (type) => {
        if(type === 'phn'){
            phnref.current.select();
            setCopyText({...copyText,phn:'Copied'});
        }else if(type === 'phnin'){
            phninref.current.select();
            setCopyText({...copyText,phnin:'Copied'});
        }else if(type === 'phnuae'){
            phnuaeref.current.select();
            setCopyText({...copyText,phnuae:'Copied'});
        }else{
    ...
    return (
        <>
        {/* Contact details popup for call */}
            <Modal size="md" show={show} onHide={Close} className="contact-modal" centered>
                    ...
                    <div id="ag_detail">
                        <div className="contact-list">
                            <div className="contact">
                                <input  ref={phnref} style={{opacity:0, position: "absolute", "z-index": -999, pointerEvents:"none"}} value={site_settings ? (site_settings.property_request_call_whatsapp ? site_settings.property_request_call_whatsapp: '') : ''} />
                                <div className="icon">
                                ...
                                <div className="contact-detail">
                                    <a className="phone_click" id="phone" href={`tel:${site_settings ? (site_settings.property_request_call_whatsapp ? site_settings.property_request_call_whatsapp: '') : ''}`}>
                                    {site_settings ? (site_settings.property_request_call_whatsapp ? site_settings.property_request_call_whatsapp: '') : ''}
                                    </a>
                                </div>
                                <div className="contact-copy">
                                    <Button variant="transparent text-color" onClick={()=>copyToClipboard('phn')}>
            ...
            <Modal show={showWhatsapp} onHide={CloseWhatsapp}>
                ...
                <Modal.Body>
                    <div id="ag_detail">
                        ...
                        <div className="contact-list">
                            <input  ref={whatsappref} style={{opacity:0, position: "absolute", "z-index": -999, pointerEvents:"none"}} value={site_settings ? (site_settings.property_request_call_whatsapp ? site_settings.property_request_call_whatsapp: '') : ''} />
                            ...
                            <div className="contact">
                                ...
                                <div className="contact-detail">
                                    ...
                                    <a target="_blank" rel="noopener noreferrer" className="phone_click" id="phone" href={"https://api.whatsapp.com/send?phone="+(site_settings ? (site_settings.property_request_call_whatsapp ? site_settings.property_request_call_whatsapp: '') : '')+"&text=Join WatsApp Group to ask question/stay"}>
                                       {site_settings ? (site_settings.property_request_call_whatsapp ? site_settings.property_request_call_whatsapp: '') : ''}
                                    </a>
                                </div>
                                <div className="contact-copy">
