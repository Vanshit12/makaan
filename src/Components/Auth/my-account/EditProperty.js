...
const EditProperty = (props) => {
    ...
    useEffect(() => {
        console.log(param.uuid,'param.idparam.idparam.id')
        window.scrollTo(0, 0);
        ...
        let endpoints = [
            ...
            `cities-list`,
            `parent-locations`,
            `fetch-property?uuid=${param.uuid}`
        ];
        axios.all(endpoints.map((endpoint,i) => {
            ...
            (res) => {
                setState({...state, features: res[0].data.data, property_types: res[1].data.data, cities: res[2].data.data, parent_locations: res[3].data.data, property_data: res[4].data.data })
            }
        );
                    ...
                    <Row className="align-items-center">
                        <Col xl lg md sm={12}>
                            <div class="myacconut_wrap_toggle">
                                <h2>{t('Add Property.Add Property')} <span> (<span className="color">*</span>) {t('Add Property.Required Fields.')}</span></h2>
                            </div>
                        </Col>
                    </Row>
                </div>
                <section className="add-property-section pt-0">
                    <div className="property-search">
                        <div className="hero_search mt-0">
                            <Card className="p-0 border-0 overflow">
                                                                            ...
                                                                            state.property_types ? (state.property_types.map((property_type, i)=>{
                                                                                return(
                                                                                        property_type.child_property_type.map((child_property_type, j)=>{
                                                                                            ...
                                                                                            }else if (purposeState.purpose === "sale" && state.parent_type === 'homes' && !(child_property_type.slug === 'room' || child_property_type.slug === 'upper-portion' || child_property_type.slug === 'lower-portion')){
                                                                                                return (
                                                                                                    child_property_type.parent_id === state.parent_type && 
                                                                                                    (<Col xs="auto" key={"child_type-"+child_property_type.slug}>
                                                                                                        <ToggleButton  type="checkbox" id={"child_type-"+child_property_type.slug}  variant="outline-primary" className="btn_radio_2"
                                            ...
                                            <Row className="justify-content-center">
                                                ...
                                                !authUser && (
                                                    <div className="login-form-link">
                                                        <p>{t("Login.Don't have an account?")} <Link to="/register"><span>{t('Login.Sign Up')}</span></Link></p>
                                                    </div>
                                                )
                                                }
                                                </Col>
